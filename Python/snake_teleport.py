...
PORTAL_A_COLOR = (0, 150, 255)   # Blue
PORTAL_B_COLOR = (255, 140, 0)   # Orange
...
# ===============================
# STATIC OBSTACLES (regenerate each food eaten)
# ===============================
def generate_static_obstacles(num, forbidden):
    """Generate obstacles avoiding forbidden cells (snake head, food, portal)."""
    obs = []
    while len(obs) < num:
        ox = random.randrange(0, WIDTH - BLOCK, BLOCK)
        oy = random.randrange(0, HEIGHT - BLOCK, BLOCK)
        if [ox, oy] not in forbidden:
            obs.append([ox, oy])
    return obs
...
def spawn_portals():
    while True:
        ax = random.randrange(0, WIDTH-BLOCK, BLOCK)
        ay = random.randrange(0, HEIGHT-BLOCK, BLOCK)
        bx = random.randrange(0, WIDTH-BLOCK, BLOCK)
        by = random.randrange(0, HEIGHT-BLOCK, BLOCK)
        if (ax, ay) != (bx, by):
            return [ax, ay], [bx, by]
...
def game_loop():
    ...
    # Portals
    portalA, portalB = spawn_portals()

    # Obstacles (static)
    obstacles = generate_static_obstacles(level + 2, forbidden=[[x,y],[food_x,food_y], portalA, portalB])
    ...
    while True:
        ...
        # Draw portals
        pygame.draw.rect(window, PORTAL_A_COLOR, [portalA[0], portalA[1], BLOCK, BLOCK])
        pygame.draw.rect(window, PORTAL_B_COLOR, [portalB[0], portalB[1], BLOCK, BLOCK])
        ...
        if special_food and x==fx and y==fy:
            ...
            # REGENERATE obstacles + portals
            portalA, portalB = spawn_portals()
            obstacles = generate_static_obstacles(
                level + 2,
                forbidden=[[x,y],[food_x,food_y], portalA, portalB]
            )
